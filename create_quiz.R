...
dec_to_rom <- function(a) {
  # l = lower, m = middle, u = upper, what letters are used for digits 0:9?
  template <- c("-", "l", "ll", "lll", "lm", "m", "ml", "mll", "mlll",  "lu")
  lookup <- do.call(cbind, lapply(c("M--", "CDM", "XLC", "IVX"), function(x) {
    lmu <- strsplit(x, "")[[1L]]
    sapply(USE.NAMES = FALSE, template, function(y) {
      gsub("-", "", gsub("l", lmu[1], gsub("m", lmu[2], gsub("u", lmu[3], y))))
    })
  }))
  digits <- matrix(a, length(a), 4) %/% t(matrix(10^(3:0), 4, length(a))) %% 10
  do.call(paste0, lapply(1:4, function(j) lookup[cbind(digits[, j] + 1, j)]))
}
